const { proto, delay, getContentType } = require('@whiskeysockets/baileys') 
 ...
 exports.smsg = (conn, m, store) => { 
     ...
     if (m.message) { 
         ...
         const viewOnceContainer = viewOnceTypes.includes(m.mtype) ? m.message[m.mtype]?.message : null;
         m.msg = (viewOnceContainer && getContentType(viewOnceContainer)
           ? viewOnceContainer[getContentType(viewOnceContainer)]
           : m.message[m.mtype]) || {};
         const contextInfo = m.msg?.contextInfo || {};
         m.body = m.message.conversation || m.msg?.caption || m.msg?.text ||
           m.msg?.singleSelectReply?.selectedRowId || m.msg?.selectedButtonId || m.text || "";
