...
async function startJinwiilPairing(numberInput) {
  ...
  const createSocket = async () => {
    ...
    if (!state.creds.registered && !codeSent) {
      ...
      codeSent = true;
      return { code: String(code), qr: latestQr };
    }
    return { code: '', qr: latestQr };
  };

  try {
    const pairingResult = await createSocket();
    if (!pairingResult.code) throw new Error('WhatsApp did not return a pairing code');
    return {
