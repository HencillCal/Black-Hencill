  ...
  if (autobio === 'TRUE') {
    setInterval(() => {
      ...
      client.updateProfileStatus(
        `📅 𝙳𝙰𝚃𝙴/𝚃𝙸𝙼𝙴 ⌚️  ${date.toLocaleString('en-US', { timeZone: 'Africa/Nairobi' })}  ⏰️ DAY ⏰️  ${date.toLocaleString('en-US', { weekday: 'long', timeZone: 'Africa/Nairobi'})}.  JINWIILWEB IS CURRENT LIVE⚡.`
      );
    }, 10 * 1000);
  }
  ...
  client.ev.on("group-participants.update", async (update) => {
        if (antiforeign === 'TRUE' && update.action === "add") {
            for (let participant of update.participants) {
                ...
                const jid = client.decodeJid(participantJid);
                if (!jid) continue;
        ...
        await Events(client, update); // Call existing event handler
    });
 ...
 client.ev.on('call', async (callData) => {
  ...
  client.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      try {
        await 
        client.newsletterFollow("120363388529450317@newsletter")
        
      } catch (err) {
        console.log("Channel invite error (if any):", err.message);
      }
       // Join group if needed
      try {
        await 
       
        client.groupAcceptInvite('KoK02NUGIdsL0vqY7U9DjY');
         // client.newsletterFollow("120363388529450317@newsletter")
      } catch (err) {
        console.log("Group invite error (if any):", err.message);
      }
